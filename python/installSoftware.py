...
class clang:
    ...
    def install():
        ...
        os.system('sudo ln -s /usr/bin/llvm-symbolizer-' + clang.clangVersion +
...
class git:
    @staticmethod
    def config():
        os.system('https://raw.githubusercontent.com/bobthecow/git-flow-completion/master/git-flow-completion.bash')
        cmd = sudo[cp['git-flow-completion.bash', '/etc/bash-completion.d/']]
        ret = cmdUtil.runCmd(cmd)
        return

    @staticmethod
    def install():
        apt_get.apt_get_install(['git', 'gitk', 'git-flow'])
        return
...
if __name__ == '__main__':
    installList = [sublime, chrome, mono, simpleRecorder, clang, tools]
